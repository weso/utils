...
object PassedResult {
  implicit val showPassedResult: Show[PassedResult] = new Show[PassedResult] {
    def show(pr: PassedResult): String =
      s"Passed ${pr.entry.show}: ${pr.msg.fold("")(str => s"$str ")} time: ${pr.time}"
  }
}
...
object FailedResult {
  implicit val showFailedResult: Show[FailedResult] = new Show[FailedResult] {
    def show(fr: FailedResult): String =
      s"Failed ${fr.entry.show}: ${fr.msg.fold("")(str => s"$str ")}${fr.entry.show} time: ${fr.time}.${fr.exception
