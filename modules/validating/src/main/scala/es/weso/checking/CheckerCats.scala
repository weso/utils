...
abstract class CheckerCats extends Checker {
  ...
  def fromIO[A](io: IO[A]): Check[A] =
    EitherT.liftF[WriterEC, Err, A](
      WriterT.liftF[ReaderEC, Log, A](Kleisli.liftF[ReaderConfig, Env, A](Kleisli.liftF[IO, Config, A](io)))
    )
  ...
  /** Given a computation check that returns a pair of value and a flag, returns the first value whose flag is true If
    * none is true, returns the value of the computation parameter last
    * @param ls
    * @param check
    * @param last
    * @tparam A
  ...
  /** Run a computation in a local environment. If the computation fails, return the result of calling `safe` function
  ...
  /** Given a list of checks, return the list of values that pass It never fails (in case of failure, it ignores the
    * value)
    */
  ...
  /** Attempts to execute a check If it fails, applies `thenPart` to the result, otherwise applies `elsePart` to the
    * error
    *
    * @param check
  ...
  /** If `c` is some value, applies `check`, otherwise applies `default`
    ...
    * @return
    */
  def optCheck[A, B](c: Option[A], check: A => Check[B], default: => Check[B]): Check[B] = c.fold(default)(check(_))
